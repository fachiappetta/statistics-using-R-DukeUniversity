...
```{r set-seed}
set.seed(5)                 # make sure to change the seed
```
...
#find upper and lower bounds of CI
lower <- sample_mean - z_star_95 * se
upper <- sample_mean + z_star_95 * se
c(lower, upper)

#true mean
params

#yes true mean is 1499.69
```
...
#99% confidence level. using signifiance level 1- a/2 = 1 - 0.10/2 = 0.95.
a = 0.99
az = (1 - a) / 2
z_star_99 = qnorm(az)
abs(z_star_99)
...
```{r plot-99-perc-cis}
#find z_star_99
lower <- sample_mean - z_star_99 * se
upper <- sample_mean + z_star_99 * se
c(lower, upper)
...
#make new column with yes or no
ci <- ci %>%
  mutate(capture_mu = ifelse(lower < params$mu & upper > params$mu, "yes", "no"))
